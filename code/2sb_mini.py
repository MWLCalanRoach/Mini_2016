...
#
    ...
    for i in range(2048):
        ...
        interleave_a.append(float(((float(a_0l[i])+1)/(scale))))#24 es el original
        #interleave_b.append(float(float(a_0m[i])+1))
        interleave_b.append(float(((float(a_0m[i])+1)/(scale))))#24 es el original
        #interleave_a.append(float(((float(a_0l[i])+1)/(2**25))))#24 es el original
...
# Se inicializan graficos y sus parametros.
    ...
    g0.clear()    
    g0.title('LSB spectrum using '+bitstream+' | Max frequency = '+str(bw)+' MHz')
    g0.xlabel('Channel #')
    ...
    g1.clear()    
    g1.title('USB spectrum using '+bitstream+' | Max frequency = '+str(bw)+' MHz')
    g1.xlabel('Channel #')
